...
public class BotTweetService {
	...
	public void favorite() throws TwitterException {
		...
		for(Status status : twitter.search(QueryFactory.create("水族館")).getTweets()){
			if(!containsPhoto(status.getMediaEntities()) || filter(filters, status) || status.getUser().getId() == twitter.getId() || twitter.showStatus(status.getId()).isFavorited() || twitter.search(QueryFactory.create(status.getText())).getTweets().size() > 1){
				continue;
			}
			try {
